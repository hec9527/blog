...
我想应该很多人都遇到过，晚上睡觉收到一条消息，拿起手机一解锁瞬间亮瞎狗眼。人眼对光线的适应需要一定的时间，突然的光线变化会让人难以适应，会感到发眩，眼睛睁不开，什么都看不清，要过几秒钟才能恢复正常，这就是光适应现象。对于需要长期面对显示器的人群而言，有一个舒适的显示亮度和光线尤为重要...

<!-- truncate -->

## 背景

在黑暗主题下，用户可以选择深色的系统外观，在弱光环境下不会太刺眼，正常光线下显示也还不错，可以提高用户的使用体验
...
css 媒体查询适配深色模式案例可以参考这个 👉🏻 <a href="/html-dark-mode/css-dark-mode.html" target="__blank">CSS 媒体查询适配深色模式</a>
...
别着急，h5 组件也是支持深色模式适配的，我们只需要在 `head` 中添加这段代码，然后将系统调整为深色模式即可查看效果了。同时在系统主题色修改的时候，和 CSS 的媒体查询一样，会自动修改 H5 组件的色彩模式

```html
<meta name="color-scheme" content="light dark" />
```

当然也可以在 CSS 中指定色彩模式，这里的 `:root` 也可以是 `body` 以及其它任何元素
...
随便提一句，在现代前端工程中，首屏加载时间是衡量页面性能的重要指标，我们可以 `<link>` 标签中按需引入 CSS，不引入无用的 CSS，从而减少数据加载量，减少首屏加载时间
...
具体怎么实现呢，通常可以在 css 中定义一些 css 变量，在应用中使用这些变量，媒体查询回调中修改 body 类名修改变量的值，最终实现主题色修改。这个案例中我们根据媒体查询列表的 `matches` 属性来确定 body 元素上应用的类名
...
我们可以在页面上添加一个复选框，标识是否使用系统主题，然后添加两个单选框，用于切换用户自定义主题，当系统主题复选框勾选的时候忽略用户自定义设置。
...
Talk is cheap, show me the code. 👉🏻 <a href='/html-dark-mode/js-dark-mode.html' target="__blank">查看 JS 实现暗色主题适配方式</a>

到这里 JS 适配深色主题似乎就完成了，可是兼容性依然有待提高，在不支持媒体查询的环境中，应用程序无法做到根据系统主题自动修改，已不存在降级方案。实际上可以根据当前系统时间来推断 `深色/浅色` 模式。开启深色模式的时间可以来自于用户输入，也可以是程序默认。通过当前系统时间来判断是否开启深色模式，作为兼容方案，这个方案不能根据当前系统主题实时做出修改。 👉🏻 <a href="/html-dark-mode/time-dark-mode.html" target="__blank">根据时间适配主题</a>
...
H5 图片深色模式案例戳这里 👉🏻 <a href="/html-dark-mode/picture-dark-mode.html" target="__blank">图片深色模式案例集合</a>
