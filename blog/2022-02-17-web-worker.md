...
总所周知 JavaScript 是单线程执行的，也就是在同一个时间只能做一件事情，如果遇到执行时间较长的脚本就会导致页面卡顿，无法及时响应用户交互。那么为什么 JavaScript 不能有多个线程呢？怎样才能提高 JavaScript 的执行效率？
...
在 `worker` 中我们无法访问 `window` 对象，直接在代码中访问 window，会报 `Uncaught ReferenceError: window is not defined`，取而代之的是使用 `this` 访问全局，在 `worker` 中的全局对象是 `DedicatedWorkerGlobalScope`，从下面截图我们可以看到 `worker` 环境中的全局变量，就是阉割到极致的 `window`
...
service worker 整体兼容性相比共享 worker 要好不少，但是生产环境任需要谨慎使用
